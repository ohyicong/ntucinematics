...
<script type="text/javascript">
	...
	const movie="The First Purge"
	const movieImage="./img/THE FIRST PURGE.jpg"
	var userSelection=[];
	...
	<span class="full" style="height:500px;">
		...
		<table class="showtimetable" cellpadding="10px">
			...
			<tr>
				...
				<?php
					$query="select timestamp from loc_address where movie_id='002' && cinema_id='001' && day='001' order by timestamp asc";
					$val=null;
		...
		<table class="showtimetable" cellpadding="10px">
			...
			<tr>
				...
				<?php
					$query="select timestamp from loc_address where movie_id='002' && cinema_id='002' && day='001' order by timestamp asc";
					$val=null;
					$result=$conn->query($query);
					while($row = mysqli_fetch_assoc($result)){
						$val[]=$row;
		...
		<table class="showtimetable" cellpadding="10px">
			...
			<tr>
				...
				<?php
					$query="select timestamp from loc_address where movie_id='002' && cinema_id='003' && day='001' order by timestamp asc";
					$result=$conn->query($query);
					$val=null;
					while($row = mysqli_fetch_assoc($result)){
						$val[]=$row;
